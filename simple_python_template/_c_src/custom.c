...
static PyTypeObject CustomType = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "custom2.Custom",
...
PyMODINIT_FUNC PyInit_custom(void)
{
  ...
  // Initialize the `CUSTOM_GLOBAL string
  PyObject* CUSTOM_GLOBAL = PyUnicode_FromString("CUSTOM_GLOBAL");
  if (CUSTOM_GLOBAL == NULL) {
    Py_DECREF(m);
    return NULL;
  }

  // Add `CUSTOM_GLOBAL` to the module
  PyModule_AddObject(m, "CUSTOM_GLOBAL", CUSTOM_GLOBAL);
